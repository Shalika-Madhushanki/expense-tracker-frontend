...
const EMPTY_ITEM = { text: 'Not a valid type', value: 'Not a valid type', disabled: true }

const SearchPicker = ({ data, items, instruction, label, onSelect }) => {
  ...
  const ModalContent = useCallback(() => {
    return (
      <List
        style={{ height: 400 }}
      >
        {filterredItems.map((i) => <List.Item
          arrowIcon={false}
  ...
  return (
    <div style={styles.container}>
      ...
      <Button block onClick={() => {
        setVisible(true);
      }}>{data ? data.text : 'Please Select'}</ Button >
